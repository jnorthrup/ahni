...
public class XmlPersistableAllele implements XmlPersistable {
    ...
    private final static String NEURON_XML_BIAS_ATTRIB = "bias";
    ...
    public String toXml() {
        ...
        if (allele instanceof NeuronAllele) {
            ...
            result.append("<")
                    .append(NEURON_XML_TAG)
                    .append(" ");
            result.append(XML_ID_TAG)
                    .append("=\"")
                    .append(allele.getInnovationId())
                    .append("\" ");
            result.append(NEURON_XML_TYPE_TAG)
                    .append("=\"")
                    .append(nAllele.getType().toString())
                    .append("\" ");
            result.append(NEURON_XML_ACTIVATION_TYPE_TAG)
                    .append("=\"")
                    .append(nAllele.getActivationType())
                    .append("\" ");
            result.append(NEURON_XML_BIAS_ATTRIB)
                    .append("=\"")
                    .append(nAllele.getBias());
            result.append("\"/>\n");
            ...
            result.append("\" ").append(CONN_XML_SRCID_TAG).append("=\"").append(cAllele.getSrcNeuronId());
            result.append("\" ").append(CONN_XML_DESTID_TAG).append("=\"").append(cAllele.getDestNeuronId());
            result.append("\" ").append(CONN_XML_WEIGHT_TAG).append("=\"").append(cAllele.getWeight()).append("\"/>\n");
        }
     ...
     *
     * @param node
     * @return <code>ConnectionAllele</code> constructed from XML
     * <code>node</code>
     * @throws IllegalArgumentException
     */
    public static ConnectionAllele connectionFromXml(Node node) throws IllegalArgumentException {
        ...
        String destIdStr = atts.getNamedItem(XmlPersistableAllele.CONN_XML_DESTID_TAG).getNodeValue();
        Long destId = Long.valueOf(destIdStr);
        ConnectionAllele result = new ConnectionAllele(new ConnectionGene(id, srcId, destId));

        String weightStr = atts.getNamedItem(XmlPersistableAllele.CONN_XML_WEIGHT_TAG).getNodeValue();
