 ...
 * 
 * SKEWED - Probability of connection being removed ranges from 0 (if weight magnitude >= max weight removed) to
 * mutation rate (if weight is 0)
 * 
 * ALL - All connections have equal likelihood of being removed, regardless of weight.
 * 
 * SMALL - Mutation occurs according to James and Tucker's "A Comparative Analysis of Simplification and
 * Complexification in the Evolution of Neural Network Topologies" paper for <a
...
public class RemoveConnectionMutationOperator extends MutationOperator implements Configurable {
	...
	public static class Strategy {
		...
		/**
		 * number of connections removed (n) is calculated based on total number of connections, and the n smallest
		 * connections are removed, excepting any whose weight is larger than <code>maxWeightRemoved</code>
		 */
		...
		/**
		 * @param s
		 * @return <code>Strategy</code> object identified by <code>s</code>, or null if no strategy matches
		 *         <code>s</code>
		 */
	...
	public void init(Properties props) throws Exception {
		setMutationRate(props.getDoubleProperty(REMOVE_CONN_MUTATE_RATE_KEY, RemoveConnectionMutationOperator.DEFAULT_MUTATE_RATE));
		maxWeightRemoved = props.getDoubleProperty(REMOVE_CONN_MAX_WEIGHT_KEY, DEFAULT_MAX_WEIGHT_REMOVED);
	 ...
	 * @see MutationOperator#MutationOperator(double)
	 */
	public RemoveConnectionMutationOperator(double aMutationRate, double aMaxWeightRemoved, Strategy aStrategy) {
		super(aMutationRate);
		maxWeightRemoved = aMaxWeightRemoved;
		strategy = aStrategy;
	}
	...
	/**
	 * Removes, in ascending order of weight magnitude, those connections whose weight magnitude is less than the
	 * maximum weight to be removed. Maximum number of connections that can be removed is determined by mutation rate.
	 * 
	...
	private void mutateAll(NeatConfiguration config, List allConns, Set allelesToRemove) {
		...
		while (it.hasNext()) {
			ConnectionAllele connAllele = (ConnectionAllele) it.next();
			if ((Math.abs(connAllele.getWeight()) <= maxWeightRemoved) && doesMutationOccur(config.getRandomGenerator()))
				allelesToRemove.add(connAllele);
		}
	}
