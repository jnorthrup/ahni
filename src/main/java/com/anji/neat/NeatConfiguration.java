...
public class NeatConfiguration extends Configuration implements Configurable {
    ...
    /**
     * properties key, proportion of new population that is generated via
     * crossover from two parents (as opposed to mutation of a single parent),
     * [0, 1].
     */
    public static final String CROSSOVER_PROPORTION_KEY = "crossover.proportion";
    /**
     * properties key, the probability that an individual produced by the
     * crossover operator will be a candidate for having mutations applied to it
     * (independent of other mutation probabilities).
     */
    ...
    /**
     * properties key, enable speciation chromosome compatibility normalisation
     * (default is false).
     */
    public final static String CHROM_COMPAT_NORMALISE_KEY = "chrom.compat.normalise";
    /**
     * properties key, specifies whether mismatched genes (disjoint or excess)
     * in chromosome distance calculations should have their (weight) values
     * added to the distance instead of adding a constant amount to the
     * difference.
     */
    ...
    /**
     * @deprecated Adjustment amounts are now calculated as a ratio of the
     * target and current species counts. Properties key, amount to change
     * speciation threshold to maintain speciation target
     */
     ...
     * default is the number of objectives defined by fitness function.
     */
    ...
    /**
     * properties key, the NaturalSelector to use to perform the selection
     * process. Default is com.anji.integration.SimpleSelector.
     */
    public final static String SELECTOR_CLASS_KEY = "selector.class";
    /**
     * properties key, enable weighted selection process. Deprecated, use
     * SELECTOR_CLASS_KEY (selector.class).
     */
    ...
    /**
     * properties key, if {@link #INITIAL_TOPOLOGY_FULLY_CONNECTED_KEY} is
     * disabled then the initial population will be generated by applying the
     * mutation operators to clones of the sample chromosome (which will just
     * contain the input and output neurons and possibly some hidden neurons).
     * The mutation rates of the operators may be temporarily increased during
     * this process by setting this property to a value greater than 1.
     */
    ...
    /**
     * properties key, allowed activation function types if the
     * INITIAL_TOPOLOGY_ACTIVATION_KEY is "random".
     */
    ...
    /**
     * properties key, if "true" then bias should be provided to the network via
     * an input, with value set by fitness function (old way of doing it), if
     * "false" then the neurons will use internal bias values (new way). Default
     * is "true".
     */
    ...
    protected void initMutation() throws InvalidConfigurationException {
        ...
        if ((!props.getBooleanProperty("training.backpropagation.enabled", false) ||
                (props.getBooleanProperty("training.backpropagation.enabled", false) && 
                props.getBooleanProperty("training.backpropagation.finalrun", false)))
        ...
        if (props.getBooleanProperty("training.backpropagation.enabled", false)) {
            ...
            backpropOperator.setFinalRun(props.getBooleanProperty("training.backpropagation.finalrun", false));
        }
    }
     ...
     * mutation. If a previous mutation has occurred adding a neuron on
     * connection connectionId, returns a neuron with that id - otherwise, a new
     * id.
     *
     * @param connectionId
     * @return NeuronAllele
     ...
     * <code>srcNeuronId</code> to neuron <code>destNeuronId</code> according to
     * NEAT add connection mutation; if a previous mutation has occurred adding
     * a connection between srcNeuronId and destNeuronId, returns connection
     * with that id; otherwise, new innovation id
     *
     * @param srcNeuronId
     * @param destNeuronId
    ...
    /**
     * Returns true if bias should be provided to the network via an input with
     * value set by fitness function (old way of doing it), or false then the
     * neurons will use internal bias values (new way).
     *
