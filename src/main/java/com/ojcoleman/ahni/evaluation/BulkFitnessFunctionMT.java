...
/**
 * <p>
 * Provides a base for multi-threaded bulk fitness functions. Provides a
 * multi-threaded framework for performing evaluations on multiple genomes. The
 * methods {@link #evaluate(Chromosome, Activator, int)} must be implemented in
 ...
 * {@link #fitnessObjectivesCount()}, {@link #noveltyObjectiveCount()} and
 * {@link #dispose()}. Subclasses may also need to override
 * {@link #getLayerDimensions(int, int)} or
 * {@link #getNeuronPositions(int, int)} to specify required layer dimensions or
 * neuron positions, for example for input or output layers.
 * </p>
 * <p>
 * Transcription and evaluation may be performed by a cluster of computers using
 * the "minion" system. In a minion set-up the initial instance of AHNI controls
 * one or more minion instances and doesn't perform any transcriptions or
 * evaluations itself. The controller and minions communicate via sockets, with
 * each minion instance acting as a server which waits for requests from the
 * controlling instance The controller automatically load-balances between the
 * minions and is robust to minions failing. See {@link #MINION_HOSTS}.
 * </p>
 * <p>
 * See {@link com.ojcoleman.ahni.evaluation.TargetFitnessFunctionMT} for an
 * example.
 * </p>
 *
...
public abstract class BulkFitnessFunctionMT extends AHNIFitnessFunction implements Configurable {
    ...
    /**
     * Property key for minimum number of threads to use for fitness evaluation
     * (including transcription of genotype/cppn to phenotype/substrate).
     */
    public static final String MIN_THREADS_KEY = "fitness.min_threads";
    /**
     * Property key for maximum number of threads to use for fitness evaluation
     * (including transcription of genotype/cppn to phenotype/substrate). If
     * value is <= 0 then which ever is lower of the detected number of
     * processor cores and the specified minimum will be used.
     */
    public static final String MAX_THREADS_KEY = "fitness.max_threads";

    /**
     * Property key for specifying additional fitness function classes in a
     * multi-objective evaluation. Note that these fitness functions must extend
     * BulkFitnessFunctionMT.
     */
    public static final String MULTI_KEY = "fitness.function.multi.class";

    /**
     * The overall fitness will be assigned to each individual according to a
     * weighted sum of the fitness values provided by each fitness function,
     * according to the weightings provided here. If not specified then each
     * objective will have equal weighting. Must be a comma-separated list of
     * weight values. The first value should correspond to the primary fitness
     * function specified by fitness_function.class followed by a weight value
     * for each function specified by fitness.function.multi.class. For all
     * fitness functions that define a novelty behaviour the weighting for these
     * must be included at the end of this list in the same order as the
     * functions are declared by fitness.function.multi.class. Note that a
     * NaturalSelector (eg NSGA-II selector) may set the overall fitness to
     * something else based on the multiple objectives, in which case the
     * weightings here will be ignored.
     */
    public static final String MULTI_WEIGHTING_KEY = "fitness.function.multi.weighting";

    /**
     * Property key for specifying whether the performance for an individual
     * should be forced to the overall fitness. Default is false.
     */
    public static final String FORCE_PERF_FITNESS = "fitness.function.performance.force.fitness";

    /**
     * <p>
     * A comma-separated list of host names and ports of minion instances. A
     * user name must be prepended if minion.autostart is enabled, in which case
     * the user name is used during the ssh login to the minion machine. A range
     * of host names may be specified by including a numeric range of the form
     * [&lt;start&gt;-&lt;end&gt;], for example hubert[09-11]:1234 (giving
     * hubert09:1234, hubert10:1234, hubert11:1234) or foo[8-10]bar (giving
     * foo8bar, foo9bar, foo10bar), note that left-padding of the numbers will
     * only occur if the &lt;start&gt; number is padded in the definition. If no
     * port is specified then minion.default_port will be used. Minions create a
     * log file called &lt;output.dir&gt;/minion.&lt;hostname&gt;.log relative
     * to the file system on the machine they're running on.</p>
     * <p>
     * Alternatively this can be set to "[htcondor]" to indicate that Minions
     * should be managed by HTCondor. Use of HTCondor forces
     * minion.autostart=TRUE. NOTE: use of HTCondor was an experiment that
     * couldn't be completed due to the restrictions of the HT Condor system
     * available for testing. There's no technical reason it shouldn't work, so
     * you can try it out, but keep in mind that it hasn't been properly
     * tested.</p>
     */
    ...
    /**
     * <p>
     * On *nix machines the controlling instance can optionally automatically
     * launch - via ssh commands - minion instances configured to serve it,
     * restarting minion instances if they die (for example if a machine is
     * reset). If this is enabled then the minion instances will be
     * automatically terminated when the controlling instance terminates.<p>
     * <p>
     * Alternatively Minions can be manually started with a command like
     * <em>java -cp &lt;jar file(s)&gt; com.ojcoleman.ahni.evaluation.Minion
     * --port 1234 --log logfilename</em></p>
     * <p>
     * This setting has no effect if minion.hosts is set to "[htcondor]".</p>
     */
    ...
    /**
     * Whether the best performance is lower of higher. If 1 then the best
     * performance is higher, if 0 then the best performance is lower.
     */
    ...
    /**
     * If greater than 1 then use an average of the best performance over this
     * many generations.
     */
    ...
    /**
     * Subclasses may override this method to perform initialise tasks.
     * <strong>Make sure to call this method from the overriding method,
     * typically at the end of it</strong>.
     *
     * @param props Configuration parameters, typically read from the/a
     * properties file.
     */
    ...
    /**
     * Calls {@link #initialiseEvaluation()} on this and all secondary fitness
     * functions.
     */
    ...
    /**
     * If required, initialise data for the current evaluation run. This method
     * is called at the beginning of {@link #evaluate(List)}. It should be
     * overridden if data (eg input and/or output patterns) need to be set-up
    ...
    /**
     * If required, perform cleanup after the current evaluation run. This
     * method is called at the end of {@link #evaluate(List)}.
	 *
     */
    ...
    /**
     * This is akin to {@link #getNoveltyObjectiveCount()} but allows for
     * handling multiple fitness functions, some of which may define multiple
     * novelty objectives. Subclasses that support novelty search should
     * override this method and return the number of behaviours this fitness
     * function defines. This default implementation returns 0.
     *
    ...
    /**
     * This is akin to {@link #getObjectiveCount()} but allows for handling
     * multiple fitness functions, some of which may define multiple objectives.
     * This default implementation returns 1. If the fitness function defines
     * more than one fitness objective, or no fitness objectives (eg only a
     * novelty objective), then this method must be overridden to return the
     * number of fitness objectives defined. NOTE: unlike
     * {@link #getObjectiveCount()} this method should not include the number of
     * novelty objectives.
     *
    ...
    /**
     * This is akin to {@link #getObjectiveLabels()} but allows for handling
     * multiple fitness functions, some of which may define multiple fitness or
     * novelty objectives. This default implementation returns an array
     * containing
     * {@link #fitnessObjectivesCount()} + {@link #noveltyObjectiveCount()}
     * strings constructed of the class name of the sub-class prefixed with
     * either F# or N# where F denotes a regular fitness objective, N denotes a
     * novelty objective and # is the index of the objective.
     */
     ...
     * {@inheritDoc}
     * <p>
     * This implementation is set as final to allow this class to make use of
     * multiple separate fitness functions for a multi-objective setup.
     * </p>
     *
     * @see #fitnessObjectivesCount()
     * @see #noveltyObjectiveCount()
    ...
    /**
     * If the fitness value(s) and behaviour(s) for novelty search (if used) for
     * this fitness function do not change between generations then subclasses
     * should override this method to return true in order to avoid
     * unnecessarily recalculating the fitness value(s) or behaviour(s). This
    ...
    private void finaliseEvaluation(Chromosome chrom) {
        ...
        // If the fitness function hasn't explicitly set any performance values, 
        // just set it to overall fitness value.
    ...
    /**
     * <p>
     * Evaluate an individual genotype on one fitness objective (if more than
     * one fitness objective is defined by this fitness function and/or it
     * defines a behaviour for novelty search then
     * {@link #evaluate(Chromosome, Activator, int, double[], Behaviour[])} must
     * be overridden instead). This method is called from
     * {@link #evaluate(List)} and must be overridden in order to evaluate the
     * genotypes.
     * </p>
     * <p>
     * If the fitness function defines a performance value then this method
     * should call {@link Chromosome#setPerformanceValue(double)}. In a
     * multi-objective set-up only the performance defined by the primary
     * fitness function will be used. If a performance value isn't defined it
     * will be set to the overall fitness value. This default implementation
     * returns 0.
     * </p>
     * <p>
     * <em>Note:</em>This method may be called simultaneously by multiple
     * threads (see {@link #MAX_THREADS_KEY}), thus any state variables that may
     * change during the course of an evaluation must be independent for each
     * thread that calls this method. The parameter evalThreadIndex may be used
     * to determine which thread is calling this method or more simply state
     * variables can be instantiated within the method. The total number of
     * threads is {@link #numThreads}.
     * <p>
     *
     * @param genotype the genotype being evaluated. This is not usually
     * required but may be useful in some cases.
     * @param substrate the phenotypic substrate of the genotype being
     * evaluated.
     * @param evalThreadIndex The index of the evaluator thread. This is not
     * usually required but may be useful in some cases.
     * @return The fitness value, in the range [0, 1]. Note that if novelty is
     * the only objective then the returned value is ignored.
     */
    ...
    /**
     * <p>
     * For fitness functions that define multiple fitness objectives (see
     * {@link #fitnessObjectivesCount()}) and/or behaviours for novelty search
     * (see {@link #noveltyObjectiveCount()}), evaluate an individual genotype
     * over all of these objectives. This default implementation simply calls
     * {@link #evaluate(Chromosome, Activator, int)}. If the fitness function
     * defines a performance value then this method should call
     * {@link Chromosome#setPerformanceValue(double)}. In a multi-objective
     * set-up (see {@link #MULTI_KEY}) only the performance defined by the
     * primary fitness function will be used. If a performance value isn't
     * defined it will be set to the (first) fitness value given by the primary
     * fitness function.
     * </p>
     * <p>
     * <em>Note:</em>This method may be called simultaneously by multiple
     ...
     * @param evalThreadIndex The index of the evaluator thread. This is not
     * usually required but may be useful in some cases.
     * @param fitnessValues An array to hold the fitness value(s) as determined
     * by this fitness function. The array will have length
     * {@link #fitnessObjectivesCount()}.
     * @param behaviours An array to hold the behaviour(s) for novelty search as
     * determined by this fitness function. The array will have length
     * {@link #noveltyObjectiveCount()}, and can be safely ignored if this
     * method returns 0.
     */
     ...
     * {@link AHNIFitnessFunction#evaluateGeneralisation(Chromosome, Activator, String, boolean, boolean)}
     * is marked as final to allow this class to handle multiple fitness
     * functions, some of which may define multiple objectives.
     ...
     * {@link AHNIFitnessFunction#evaluateGeneralisation(Chromosome, Activator, String, boolean, boolean)}
     * but allows for handling multiple fitness functions, some of which may
     * define multiple objectives. This method may optionally be overridden to
     * perform an evaluation that performs a generalisation test of an
     * individual genotype and output the results to a log. The generalisation
     * test should use training examples or environments that are not used for
     * fitness evaluations. The evaluation is not used for any purpose other
     * than generating a record of the generalisation performance of the given
     * genotype. The function should set the performance values and fitness
     * values of the given Chromosome using the functions setFitnessValue(...)
     * and setPerformanceValue(...).
     *
     * @param genotype the genotype being evaluated. This is not usually
     * required but may be useful in some cases.
     * @param substrate the phenotypic substrate of the genotype being
     * evaluated.
     * @param baseFileName The base/prefix of the names of log file(s) that
     * should be created.
     ...
     * by this fitness function. The array will have length
     * {@link #fitnessObjectivesCount()}.
     * @return Implementations of this method must return true. This default
     * implementation returns false to indicate that it's a dummy
     * implementation.
     */
    ...
    protected class Evaluator extends Thread {
        ...
        /**
         * Deletes the current substrate, a completely new one will be
         * generated. This is useful for when a substrate can be reused by the
         * Transcriber, but sometimes needs to be completely regenerated.
         *
    ...
    /**
     * A convenience method to generate a substrate from the given Chromosome
     * using the configured transcriber for this fitness function.
     *
     * @param chrom The Chromosome to generate the substrate from.
     * @param substrate If a substrate can be reused for efficiency reasons it
     * can be supplied here, otherwise null may be given.
     ...
     * {@inheritDoc}
     *
     * This implementation will generate images for the novelty archive(s) if
     * possible. If called multiple times in the same generation it will only
     * generate images the first time. If overridden, subclasses should call
     * this method from their own implementation.
     */
    @Override
    ...
    /**
     * Sub-classes may override this method to dispose of resources upon
    ...
    /**
     * Sub-classes may override this method to perform operations after a
     * Chromosome has been evaluated.
     *
     * @param genotype the Chromosome that has been evaluated. It's fitness and
     * performance will have been set.
     * @param substrate the network (activator), or phenotype, of the evaluated
     * Chromosome.
     * @param evalThreadIndex The index of the evaluator thread. This is not
     * usually required but may be useful in some cases.
     */
