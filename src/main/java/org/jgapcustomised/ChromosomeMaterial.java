 ...
 * can be modified by genetic operators before creating the Chromosome object.
 * Also enables us to handle special cases, like sample chromosome, where you
 * don't need a Configuration or fitness value. Also, made methods not
 * synchronized, since only Genotype.evolve() should be modifying this object.
 */
public class ChromosomeMaterial implements Comparable, Serializable {
    ...
    /**
     * Create chromosome with no parents. Used for startup sample chromosome
     * material.
     *
     * @param a_initialAlleles
     * @see ChromosomeMaterial#ChromosomeMaterial(Collection, Long, Long)
    ...
    /**
     * Returns a copy of this ChromosomeMaterial. The returned instance can
     * evolve independently of this instance.
     *
     * @param parentId represents ID of chromosome that was cloned. If this is
     * initial chromosome material without a parent, or is the clone of material
     * only (e.g., before the it has become a Chromosome), parentId == null.
    ...
    /**
     * Returns the size of this ChromosomeMaterial (the number of alleles it
     * contains).
     *
     * @return The number of alleles contained within this ChromosomeMaterial
     * instance.
     */
    public int size() {
    ...
    /**
     * Retrieves the set of genes. This method exists primarily for the benefit
     * of GeneticOperators that require the ability to manipulate Chromosomes at
     * a low level.
     *
    ...
    /**
     * Returns a string representation of this Chromosome, useful for some
     * display purposes.
     *
    ...
    /**
     * Convenience method that returns a new Chromosome instance with its genes
     * values (alleles) randomized. Note that, if possible, this method will
     * acquire a Chromosome instance from the active ChromosomePool (if any) and
     * then randomize its gene values before returning it. If a Chromosome
     * cannot be acquired from the pool, then a new instance will be constructed
     * and its gene values randomized before returning it.
     *
     ...
     * is invalid.
     * @throws IllegalArgumentException if the given Configuration instance is
     * null.
     */
     ...
     * if and the argument is an instance of the Chromosome class and has a set
     * of genes equal to this one.
     *
     ...
     * Compares this Chromosome against the specified object. The result is true
     * if and the argument is an instance of the Chromosome class and has a set
     * of genes with equal values (eg connection weight, activation type) to
     * this one.
     *
     * @param other The object to compare against.
     * @return true if the objects are the same, false otherwise.
     ...
     * chromosome, zero if they are equal to each other, and a positive number
     * if this chromosome is "greater than" the given chromosome.
    ...
    /**
     * @return primary parent ID; dominant parent if chromosome spawned by
     * crossover
     */
    ...
    /**
     * @return primary parent ID; recessive parent if chromosome spawned by
     * crossover
     */
    ...
    /**
     * returns excess alleles, defined as those alleles whose innovation ID is
     * greater than <code>threshold</code>; also, removes excess alleles from
     * <code>alleles</code>
     *
     * @param alleles <code>List</code> contains <code>Allele</code> objects
     * @param threshold
     * @return excess alleles, <code>List</code> contains <code>Allele</code>
     * objects
     */
     ...
     * </a> speciation methodology. Made it generic enough that the genes do not
     * have to be nodes and connections.
     *
     * @param target
    ...
    /**
     * Indicates whether this individual is a candidate for mutations (after
     * reproduction). Default value is true.
     *
