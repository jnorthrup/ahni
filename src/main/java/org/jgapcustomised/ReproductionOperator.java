...
/**
 * Abstract class for reproduction operators. Handles intra-species breeding and
 * someday will handle inter-species breeding. Each specie gets a number of
 * offspring relative to its fitness, following <a
...
public abstract class ReproductionOperator {
     ...
     * operators referenced by the current Configuration object during the
     * evolution phase. Operators are given an opportunity to run in the order
     * they are added to the Configuration. Iterates over species and determines
     * each one's number of offspring based on fitness, then passes control to
     * subclass <code>reproduce( final Configuration config,
     * final List parents, int numOffspring, List offspring )</code> method to
     * perform specific reproduction.
     *
     * @param config The current active genetic configuration.
     * @param parentSpecies <code>List</code> contains <code>Species</code>
     * objects containing parent chromosomes from which to produce offspring.
     * @param offspring <code>List</code> contains offspring
     * <code>ChromosomeMaterial</code> objects; this method adds new offspring
     * to this list
    ...
    final public void reproduce(final Configuration config, final List<Species> parentSpecies, List<ChromosomeMaterial> offspring) throws InvalidConfigurationException {
        ...
        // Math.max() workaround for population.size=1
        double tmp = (config.getPopulationSize() - totalEliteCount) * getSlice();
        final int targetNewOffspringCount = Math.max(1, (int)Math.round(tmp));
    ...
    /**
     * @return double slice of population this reproduction operator will fill
     * with offspring
     */
    ...
    /**
     * @param aSlice slice of population this reproduction operator will fill
     * with offspring
     */
    ...
    /**
     * Set the probability that individuals produced by this operator will be a
     * candidate for having mutations applied to them (independent of the
     * mutation rates of MutationOperators).
     *
