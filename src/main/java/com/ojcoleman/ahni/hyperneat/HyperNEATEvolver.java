...
public class HyperNEATEvolver implements Configurable, GeneticEventListener {
    ...
    private final List<AHNIEventListener> listeners = new ArrayList<>();
    ...
    /**
     * The best performing Chromosome from each generation. Sometimes this is
     * equivalent to fittest depending on whether the fitness function defines
     * performance independently.
     */
    protected Chromosome[] bestPerformingChromosomes;
    /**
     * The highest fitness achieved in each generation, as a fraction of the
     * maximum possible fitness. This is maintained as well as
     * fittestChromosomes because the fitness achieved can vary when the fitness
     * function is stochastic.
     */
    protected double[] bestFitnesses;
    /**
     * The highest performance achieved in each generation. This is maintained
     * as well as bestPerformingChromosomes because the performance achieved can
     * vary when the fitness function is stochastic.
     */
    ...
    /**
     * Construct new evolver with given properties. See
     * <a href=" {@docRoot} /params.htm" target="anji_params">Parameter Details
     * </a> for specific property settings.
     *
    ...
    /**
     * Perform a single run. This is synchronised as the object contains state
     * variables (other than configurable properties) and so multiple runs
     * should not be performed in parallel. This is in contrast to most
     * {@link Configurable} objects that do not contain state variables other
     * than configurable properties.
     *
     * @return An array containing the best performance achieved in each
     * generation.
     * @throws Exception
     */
    ...
    /**
     * Returns an array containing the best performing Chromosome from each
     * generation. This may be equivalent to the fittest Chromosomes depending
     * on whether the fitness function defines performance independently.
     */
    ...
    /**
     * Returns an array of the highest fitness achieved, as a fraction of the
     * maximum possible fitness, in each generation. This is maintained as well
     * as the fittest Chromosomes because the fitness achieved can vary when the
    ...
    /**
     * Returns an array of the highest performance achieved in each generation.
     * This is maintained as well as best performing Chromosomes because the
     * performance achieved can vary when the fitness function is stochastic.
     */
            ...
            try {
                ...
                if (substrate == null) {
                    ...
                    if (bulkFitnessFunc instanceof AHNIFitnessFunction) {
                        ...
                        if (!Double.isNaN(origFitness)) {
                            champ.setFitnessValue(origFitness);
                        }
    ...
    /**
     * @return The best performing Chromosome from the last generation. This may
     * be equivalent to the fittest Chromosome depending on whether the fitness
     * function defines performance independently.
     */
    ...
    /**
     * Provides a translation from and passes through
     * {@link org.jgapcustomised.event.GeneticEvent}s to
     * {@link com.ojcoleman.ahni.event.AHNIEvent}s via
     * {@link com.ojcoleman.ahni.event.AHNIEventListener}s.
     */
    @Override
    ...
    /**
     * Sets the current generation number, used by
     * {@link com.ojcoleman.ahni.evaluation.Minion}.
     */
    ...
    /**
     * Returns true iff evolution has finished or iff the current generation is
     * the last one.
     */
