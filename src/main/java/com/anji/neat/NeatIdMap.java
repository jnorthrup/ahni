...
public class NeatIdMap {
     ...
     * Details </a> for specific property settings.
     *
     * @param props configuration parameters; newProps[SURVIVAL_RATE_KEY] should
     * be < 0.50f
     */
     ...
     * @param srcNeuronId
     * @param destNeuronId
     * @return return id of previous connection, if any, that mutated from
     * neuron <code>srcNeuronId</code> to neuron <code>destNeuronId</code>
     */
    ...
    /**
     * store mapping between connection <code>connectionId</code> and the neuron
     * that replaced it via NEAT add neuron mutation, <code>newNeuronId</code>
     *
     * @param connectionId
     * @param newNeuronId
     */
    ...
    /**
     * store mapping between neurons <code>srcNeuronId</code> and
     * <code>destNeuronId</code> and the connection that mutated between them
     * via NEAT add connection mutation, <code>newConnectionId</code>
     *
     * @param srcNeuronId
     * @param destNeuronId
    ...
    public void load() throws IOException {
        if (neatIdMapFileName != null) {
            try (FileInputStream in = new FileInputStream(neatIdMapFileName)) {
                neatIdMapFromXml(in);
            } catch (Exception e) {
    ...
    public boolean store() throws IOException {
        if (neatIdMapFileName != null) {
            try (FileWriter out = new FileWriter(neatIdMapFileName)) {
                out.write(toXml());
                out.flush();
                return true;
            }
    ...
    /**
     * Convert NEAT ID mappings (enables re-use of innovation IDs) to XML
     * string.
     *
     * @return XML string
     */
