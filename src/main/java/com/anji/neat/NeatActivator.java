...
/**
 * Transcribe <code>Chromosome</code> object (loaded from persistence if
 * necessary) into <code>Activator</code> object and activate it with specified
 * stimuli.
 *
...
public class NeatActivator implements Configurable {
    ...
    private static final Logger logger = Logger.getLogger(NeatActivator.class);

    private Persistence db;

    private final List idxs = new ArrayList();
    ...
    public void init(Properties props) {
        try {
            ...
            stimuli = Properties.loadArrayFromFile(props.getResourceProperty(TargetFitnessFunction.STIMULI_FILE_NAME_KEY));
            targets = Properties.loadArrayFromFile(props.getResourceProperty(TargetFitnessFunction.TARGETS_FILE_NAME_KEY));

            if (stimuli.length == 0 || targets.length == 0) {
                throw new IllegalArgumentException("require at least 1 training set for stimuli [" + stimuli.length + "] and targets [" + targets.length + "]");
            }
            if (stimuli.length != targets.length) {
                throw new IllegalArgumentException("# training sets does not match for stimuli [" + stimuli.length + "] and targets [" + targets.length + "]");
            }
    ...
    /**
     * Load chromosome from persistencem transcribe it into activator, and
     * activate it.
     *
     * @param chromId persistence ID of chromosome
    ...
    public String displayActivation(String chromId) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            double[] response = (double[]) responses.get(i);
    ...
    /**
     * Loads chromosome from persistence, transcribes it into activator, and
     * displays activation.
     *
