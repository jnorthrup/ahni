...
/**
 * The Configuration class represents the current configuration of plugins and
 * flags necessary to execute the genetic algorithm (such as fitness function,
 * natural selector, genetic operators, and so on).
 * <p>
 * Note that, while during setup, the settings, flags, and other values may be
 * set multiple times. But once the lockSettings() method is invoked, they
 * cannot be changed. The default behavior of the Genotype constructor is to
 * invoke this method, meaning that once a Configuration object is passed to a
 * Genotype, it cannot be subsequently modified. There is no mechanism for
 * unlocking the settings once they are locked.
 * <p>
 * Not all configuration options are required. See the documentation for each of
 * the respective mutator methods to determine whether it is required to provide
 * a value for that setting, and what the setting will default to if not.
 */
public class Configuration implements java.io.Serializable {
     ...
     * chromosomes during the natural selection process. Note that only this or
     * the bulk fitness function may be set--the two are mutually exclusive.
     */
    private FitnessFunction m_objectiveFunction = null;

    /**
     * References the current bulk fitness function that will be used to
     * evaluate chromosomes (in bulk) during the natural selection process. Note
     * that only this or the normal fitness function may be set--the two are
     * mutually exclusive.
     */
    ...
    /**
     * References ChromosomeMaterial that serves as a sample of the Gene setup
     * that is to be used. Each gene in ChromosomeMaterial should be represented
     * with the desired Gene type. Added by Tucker and James.
     */
    private ChromosomeMaterial m_sampleChromosomeMaterial = null;

    /**
     * References the random number generator implementation that is to be used
     * for the generation of any random numbers during the various genetic
     * operations and processes.
     */
    private Random m_randomGenerator = null;

    /**
     * References the EventManager that is to be used for the notification of
     * genetic events and the management of event subscribers.
     */
    private EventManager m_eventManager = null;

    /**
     * Stores all of the ReproductionOperator implementations that are to be
     * used to operate upon the chromosomes of a population after natural
     * selection. In general, operators will be executed in the order that they
     * are added to this list. Added by Tucker and James.
     */
    private List<ReproductionOperator> reproductionOperators = new ArrayList();

    /**
     * Stores all of the MutationOperator implementations that are to be used to
     * operate upon the chromosomes of a population after natural selection. In
     * general, operators will be executed in the order that they are added to
     ...
     *
     * @throws InvalidConfigurationException if the mutation operator is null
     * a_operatorToAdd this object is locked.
     */
    ...
    /**
     * Indicates whether the settings of this Configuration instance have been
     * locked. Prior to locking, the settings may be set and reset as desired.
     * Once this flag is set to true, no settings may be altered.
     */
    private boolean m_settingsLocked = false;

    /**
     * Sets the fitness function to be used for this genetic algorithm. The
     * fitness function is responsible for evaluating a given Chromosome and
     ...
     * which Chromosome instances will be allowed to move on to the next round
    ...
    /**
     * Sets the bulk fitness function to be used for this genetic algorithm. The
     * bulk fitness function may be used to evaluate and assign fitness values
     * to the entire group of candidate Chromosomes in a single batch. This can
     * be useful in cases where it's difficult to assign fitness values to a
     * Chromosome in isolation from the other candidate Chromosomes.
     * <p>
     * Note that it is illegal to set both a bulk fitness function and a normal
     * fitness function. Although one or the other is required, the two are
     * mutually exclusive.
     *
    ...
    /**
     * Sets sample ChromosomeMaterial that is to be used as a guide for the
     * construction of other Chromosomes. ChromosomeMaterial should be setup
     * with each gene represented by the desired concrete Gene implementation
     * for that gene position (locus). Anytime a new Chromosome is created, it
     * will be constructed with the same Gene setup as that provided in this
     * sample ChromosomeMaterial.
     *
     * @param a_sampleChromosomeMaterial ChromosomeMaterial to be used as the
     * sample.
    ...
    /**
     * Retrieves sample ChromosomeMaterial that contains the desired Gene setup
    ...
    /**
     * Sets the random generator to be used for this genetic algorithm. The
     * random generator is responsible for generating random numbers, which are
     * used throughout the process of genetic evolution and selection. This
     * setting is required.
     *
    ...
    /**
     * Sets the population size to be used for this genetic algorithm. The
     * population size is a fixed value that represents the number of
     * Chromosomes contained within the Genotype (population). This setting is
     * required.
     *
     ...
     * configuration.
     *
     * @throws InvalidConfigurationException if the event manager is null or
     * this Configuration object is locked.
     */
    ...
    /**
     * Retrieves the EventManager associated with this configuration. The
     * EventManager is responsible for the management of event subscribers and
     * event notifications.
     *
    ...
    /**
     * Locks all of the settings in this configuration object. Once this method
     * is successfully invoked, none of the settings may be changed. There is no
     * way to unlock this object once it is locked.
     * <p>
     ...
     * options have been properly set. If it detects a problem, it will throw an
     * InvalidConfigurationException and leave the object unlocked.
     * <p>
     * It's possible to test whether is object is locked through the isLocked()
     * method.
     * <p>
     * It is ok to lock an object more than once. In that case, this method does
     * nothing and simply returns.
     *
     * @throws InvalidConfigurationException if this Configuration object is in
     * an invalid state at the time of invocation.
     */
     ...
     *
     * @return true if this object has been locked by a previous successful call
     * to the lockSettings() method, false otherwise.
     */
     ...
     * fact, been set. If this object is not in a valid state, then an exception
     * will be thrown detailing the reason the state is not valid.
     *
    ...
    public synchronized void verifyStateIsValid() throws InvalidConfigurationException {
        ...
        if (totalSlices != 1.0f) {
            throw new InvalidConfigurationException("Survival rate and reproduction rates are more than 1.0: " + totalSlices);
        }
    ...
    /**
     * Makes sure that this Configuration object isn't locked. If it is, then an
     * exception is thrown with an appropriate message indicating that settings
     * in this object may not be altered. This method should be invoked by any
     * mutator method in this object prior to making any state alterations.
     *
    ...
    /**
     * @return Returns the number of objectives being employed. This is only
     * valid for fitness functions and selectors that handle multiple
     * objectives.
     */
    ...
    /**
     * @return Returns the number of novelty objectives being employed. This is
     * only valid for fitness functions that define novelty behaviours.
     */
